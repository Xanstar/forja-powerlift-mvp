...
2. **Build antes de push**: corré `npm run build` y arreglá los errores vos mismo.
...
4. **Vercel**: si hay que desplegar, usá la CLI de Vercel (`npx vercel`). Lo único que necesitás que provea el usuario: un login/token de Vercel (ver dashboards). Las env vars a setear en producción: `DATABASE_URL`, `TURSO_AUTH_TOKEN`, `AUTH_SECRET`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `AUTH_TRUST_HOST=true`.
...
- `npm run dev` / `npm run build` / `npm run db:migrate` / `npm run db:seed`
