...
import { AthleteToday } from "@/components/athlete-today";
import { PinPad } from "@/components/pin-pad";
import { ForjaLogo } from "@/components/forja-logo";
import { ThemeControl } from "@/components/theme-control";
import { legacyPinAccessEnabled } from "@/lib/athlete-activation";
import { athleteForCredential } from "@/lib/server-authorization";

export default async function HoyPage() {
  const athlete = await athleteForCredential();
  if (athlete) return <AthleteToday atleta={athlete} />;
    ...
    <main className="grid min-h-screen bg-background md:grid-cols-[minmax(0,1fr)_minmax(22rem,0.72fr)]">
      <section className="hidden border-r border-on-brand-border bg-brand-canvas p-10 text-on-brand md:flex md:flex-col md:justify-between">
        ...
        <div>
          ...
          <p className="mt-6 max-w-md leading-7 text-on-brand-muted">Ingresá con la credencial segura que recibiste al activar tu acceso.</p>
        </div>
      ...
      <section className="flex min-h-screen flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-6 py-12">
          <h2 className="text-center font-display text-3xl font-bold tracking-[-0.04em]">Ingresá a tu entrenamiento</h2>
          <PinPad legacyEnabled={legacyPinAccessEnabled()} />
        </div>
      </section>
    </main>
  );
